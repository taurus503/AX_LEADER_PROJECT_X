...
function buildConfidenceExplanation(regime, input, score) {
  ...
  const newsItems = Array.isArray(newsContext.topItems) ? newsContext.topItems : [];
  const reasons = [
    {
      impact: "neutral",
      title: "기준 점수",
  ...
  if (newsItems.length) {
    newsItems.slice(0, 3).forEach((item, index) => {
      reasons.push({
        impact: input.newsSentiment === "positive" ? "plus" : input.newsSentiment === "negative" ? "minus" : "neutral",
        title: `뉴스 근거 ${index + 1}`,
        text: `${item.title} · ${item.source || "Google News"} · ${item.publishedAt || newsContext.asOf || ""}`
      });
    });
  } else {
    reasons.push({
      impact: "neutral",
      title: "뉴스 근거",
      text: `뉴스 데이터는 ${newsContext.asOf || "기준일"} 기준으로 반영되며, 추가 기사 근거는 제한적입니다.`
    });
  }
