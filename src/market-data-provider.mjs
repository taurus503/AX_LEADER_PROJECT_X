...
export function buildYahooChartUrl(symbol = "^KS200", range = "3mo", interval = "1d", options = {}) {
  ...
  if (!options.period1 && !options.period2) url.searchParams.set("range", range);
...
function pointAtOrBefore(points, dateInput) {
  ...
  if (!dateInput) return points.at(-1) || null;
...
export function computeFeaturesFromPrices(parsed, options = {}) {
  ...
  return {
    ...
    asOf: latestPoint?.date || parsed.points.at(-1)?.date || getDefaultTradingDate(),
    ...
    returns: {
      ...
      oneMonth: pctChange(latestPoint?.close, monthPoint?.close)
    },
    ...
    newsSentiment: "mixed",
    breadth: monthPoint && latestPoint && latestPoint.close > monthPoint.close + 1 ? "strong" : latestPoint && monthPoint && latestPoint.close < monthPoint.close - 1 ? "weak" : "neutral",
    trendStrength: Math.max(0, Math.min(100, Math.round(50 + pctChange(latestPoint?.close, monthPoint?.close) * 3))),
