...
let newsOffset = 0;
let suppressDateBroadcast = false;
let pendingExternalDate = "";
const attributionFrame = () => document.getElementById("attribution-frame");
...
function renderHeroMetrics(data) {
  const metrics = [
    ["현재 국면", data.regime.label],
    ["확신도", formatPercent(data.confidence.score * 100, 0)],
    ["이벤트 리스크", data.input.eventRisk],
    ["시장 해석", data.marketInterpretation],
    ["최종 업데이트", data.marketMeta.lastUpdate || data.input.asOf]
  ];
  ...
  byId("hero-regime").innerHTML = `
    <div>
      ...
      <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:14px;">
        <span class="event-pill">이벤트 리스크: ${escapeHtml(data.input.eventRisk)}</span>
        <span class="tag-pill medium">추세: ${data.input.returns.oneWeek >= 0 ? "상승 / 횡보" : "하락 / 횡보"}</span>
...
function renderMarketSnapshot(data) {
  ...
  const items = [
    ...
    ["KOSPI200 종가", meta.latestClose ?? "N/A", "직전 거래일 기준으로 반영"],
    ["직전일 종가", meta.previousTradingDayClose ?? "N/A", "변화율 계산 기준"],
    ["1일 수익률", meta.returns?.oneDay ?? data.input.returns.oneDay, "전일 대비 변화"],
    ["20일 변동성", meta.volatility20d ?? data.input.volatility20d, "최근 20일 실현 변동성"],
    ["뉴스 심리", meta.newsSentiment || data.input.newsSentiment, "최근 리포트 반영"],
...
function renderModule1Kpis(data) {
  const kpis = [
    ["국면", data.regime.label, "현재 시장 국면"],
    ["확신도", formatPercent(data.confidence.score * 100, 0), "판단 확신도"],
    ["이벤트 필터", data.input.eventRisk, "이벤트 리스크 수준"],
    ["해석", "Readable", data.marketInterpretation],
    ["방어형", data.strategy.defensive.join(", "), "방어 우선 전략"],
    ["추세형", data.strategy.momentum.join(", "), "추세 추종 전략"],
    ["캐리형", data.strategy.carry.join(", "), "캐리 / 프리미엄 전략"],
    ["균형형", data.strategy.balanced.join(", "), "균형형 전략"]
  ];
...
function renderValidation(data) {
  ...
  byId("validation-input-panel").innerHTML = `
    ...
    <div class="validation-input-grid">
      ...
      <div class="validation-input-item">
        ...
        <em>${escapeHtml(validation.backtestAvailable ? "검증 구간 확보" : "검증 구간 부족")}</em>
      </div>
      <div class="validation-input-item">
        <span>최대손실 제한</span>
        <strong>${validation.maxLossDefined ? "정의됨" : "미정"}</strong>
        <em>${escapeHtml(validation.maxLossDefined ? "손실 한도 규정 있음" : "무한손실 가능성 점검 필요")}</em>
      </div>
      <div class="validation-input-item validation-input-item--wide">
        ...
        <em>${escapeHtml(validation.integrationHint?.oneLineForCard || "Module 2 결과를 Module 3에서 검증합니다.")}</em>
      </div>
    </div>
  ...
  byId("validation-result-panel").innerHTML = `
    ...
    <div class="validation-chip-row">
      <span class="validation-chip">${escapeHtml(validation.marketAlignment?.checks?.[0]?.note || "시장 정합")}</span>
      <span class="validation-chip">${escapeHtml(validation.marketAlignment?.checks?.[1]?.note || "상세 분석")}</span>
      <span class="validation-chip">${escapeHtml(validation.marketAlignment?.checks?.[2]?.note || "국면 적합도")}</span>
    ...
    <div class="validation-summary-box">
      ...
      <div class="validation-summary-box__text">${escapeHtml(validation.comment || "검증 요약을 표시합니다.")}</div>
    </div>

    <div class="validation-duo">
      <article class="validation-info-box">
        <div class="validation-info-box__title">유리한 상황</div>
        <div class="validation-info-box__text">${escapeHtml(validation.beginnerNote || "현재 국면에서 유리한 조건")}</div>
      </article>
      <article class="validation-info-box">
        <div class="validation-info-box__title">주의할 점</div>
        <div class="validation-info-box__text">${escapeHtml(validation.riskWarning || "주의가 필요한 요인")}</div>
      </article>
    </div>

    <div class="validation-duo">
      <article class="validation-info-box">
        <div class="validation-info-box__title">Module 2 시장상태 반영</div>
        <ul class="validation-list">
          ${checks.map((check) => `<li>${escapeHtml(check.item)}: ${escapeHtml(check.note)}</li>`).join("")}
        </ul>
      </article>
      <article class="validation-info-box">
        <div class="validation-info-box__title">Module 1 통합 힌트</div>
        <ul class="validation-list">
          <li>${escapeHtml(validation.integrationHint?.targetSlot || "strategy-card-validation")}</li>
          <li>${escapeHtml(validation.integrationHint?.codex1Action || "전략 카드 하단에 결과를 표시합니다.")}</li>
          <li>${escapeHtml(validation.integrationHint.oneLineForCard || "추가 설명이 필요한 경우 한 줄로 보여줍니다.")}</li>
        </ul>
      </article>
    </div>
...
function renderAttribution(data) {
  ...
  const items = [
    ["자본배분 효과", `${data.attribution.allocationEffect.toFixed(1)}%`],
    ["선택 효과", `${data.attribution.selectionEffect.toFixed(1)}%`],
    ["상호작용 효과", `${data.attribution.interactionEffect.toFixed(1)}%`],
...
${data.attribution.betaNote || ""}`],
    ["레짐 정합성", `${data.attribution.regimeFit?.summary || "미정"}
Fit score ${Number(data.attribution.regimeFit?.score || 0).toFixed(2)}`],
    ["마이크로 상태", `${data.attribution.postHoc ? `S${data.attribution.postHoc.code} · ${data.attribution.postHoc.cluster}` : "N/A"}
...
function renderCommittee(data) {
  byId("committee-summary").innerHTML = `
    ...
    <div class="committee-items">
      <div class="committee-item"><span>추천</span><strong>${escapeHtml(data.committee.recommendation)}</strong></div>
      <div class="committee-item"><span>핵심 리스크</span><strong>${escapeHtml(data.committee.keyRisk)}</strong></div>
      <div class="committee-item"><span>백테스트 요약</span><strong>${escapeHtml(data.committee.backtestSummary)}</strong></div>
      <div class="committee-item"><span>위원회 코멘트</span><strong>${escapeHtml(data.committee.comment)}</strong></div>
    </div>
  `;

  byId("battle-plan").innerHTML = `
    <div class="battle-plan__row">
      <div class="battle-plan__label">추천 행동</div>
      <div class="battle-plan__value">${escapeHtml(data.battlePlan.recommendedAction)}</div>
    </div>
    <div class="battle-plan__row">
      <div class="battle-plan__label">예상 결과</div>
      <div class="battle-plan__value">${escapeHtml(data.battlePlan.expectedOutcome)}</div>
    </div>
    <div class="battle-plan__row">
      <div class="battle-plan__label">최악의 경우</div>
      <div class="battle-plan__value">${escapeHtml(data.battlePlan.worstCase)}</div>
    </div>
    <div class="battle-plan__row">
      <div class="battle-plan__label">자본 배분</div>
      <div class="battle-plan__value">${escapeHtml(data.battlePlan.capitalAllocation)}</div>
    </div>
    <div class="battle-plan__row">
      <div class="battle-plan__label">리스크 예산</div>
...
function readQuestionAnswer() {
  ...
  if (!byId("beginner-answer")) return "";
...
async function loadNews(offset = 0, { useSampleFallback = true, asOf = "" } = {}) {
  const requestedDate = normalizeDateInput(asOf) || normalizeDateInput(byId("market-date-input").value) || getDefaultTradingDate();
  try {
    const response = await fetch(`/api/market-news?offset=${offset}&asOf=${encodeURIComponent(requestedDate)}`, { cache: "no-store" });
    const payload = await response.json();
    ...
    newsState = {
      ...
      source: payload.source,
      note: payload.note || "",
      asOf: payload.data?.asOf || requestedDate
    };
    newsOffset = offset;
    return newsState;
  } catch (error) {
    ...
    const items = Array.isArray(sampleNews.items)
      ? sampleNews.items.map((item) => ({ ...item, publishedAt: requestedDate }))
      : [];
    const pageSize = 3;
    const topItems = items.slice(offset, offset + pageSize);
    newsState = summarizeNewsContext(topItems, { offset, pageSize, asOf: requestedDate });
    newsState.allItems = items;
    newsState.keywords = sampleNews.keywords || newsState.keywords;
    newsState.asOf = requestedDate;
...
async function runAgent() {
  ...
  renderMain();
  readQuestionAnswer();
  if (!suppressDateBroadcast) {
    window.parent?.postMessage(
      {
        type: "regime-date-sync",
        asOfDate: input.asOf,
        source: "regime"
      },
      "*"
  ...
  attributionFrame()?.contentWindow?.postMessage(
    {
      type: "attribution-date-sync",
      asOfDate: input.asOf,
      source: "regime"
    },
    "*"
...
function wireEvents() {
    ...
    if (event.target.matches("#market-date-input")) {
      ...
      if (!suppressDateBroadcast) {
        window.parent?.postMessage(
          {
            type: "regime-date-sync",
            asOfDate: event.target.value || getDefaultTradingDate(),
            source: "regime"
          },
          "*"
        );
  ...
  document.addEventListener("change", (event) => {
    if (event.target.matches("#market-date-input")) {
      const chosenDate = normalizeDateInput(event.target.value) || getDefaultTradingDate();
      currentScenario = "custom";
      setText("scenario-status", "Custom");
      renderScenarioButtons();
      void syncFromExternalDate(chosenDate);
    }
  });
...
function buildInitialSnapshot(asOfDate) {
  const normalizedDate = normalizeDateInput(asOfDate) || getDefaultTradingDate();
  const sampleMarket = sampleData?.market || sampleData?.marketData || {};
  const sampleNews = sampleData?.news || {};
  const sampleItems = Array.isArray(sampleNews.items) ? sampleNews.items : [];

  marketState = {
    ...sampleMarket,
    asOf: normalizedDate,
    source: sampleMarket.source || "Sample market data",
    note: sampleMarket.note || `초기 화면은 ${normalizedDate} 기준 샘플 데이터를 먼저 보여줍니다.`
  };

  newsState = summarizeNewsContext(sampleItems.slice(0, 3), { offset: 0, pageSize: 3 });
  newsState.allItems = sampleItems.map((item) => ({ ...item, publishedAt: normalizedDate }));
  newsState.keywords = sampleNews.keywords || newsState.keywords;
  newsState.asOf = normalizedDate;
  newsState.note = sampleNews.note || `초기 화면은 ${normalizedDate} 기준 뉴스 샘플을 먼저 보여줍니다.`;
  newsOffset = 0;

  suppressDateBroadcast = true;
  byId("market-date-input").value = normalizedDate;
  applyScenarioInputs("transition");
  syncFieldsFromMarket(marketState);
  renderScenarioButtons();
  analysis = buildAnalysis(buildInputState());
  analysis.marketMeta = marketState;
  analysis.workflow = ["Observe", "Orient", "Decide", "Act", "Feedback", "Update Signal"];
  analysis.input = buildInputState();
  renderMain();
  readQuestionAnswer();
  suppressDateBroadcast = false;

  return normalizedDate;
}

async function hydrateLiveData(asOfDate) {
  const normalized = normalizeDateInput(asOfDate) || getDefaultTradingDate();
  try {
    await loadMarket(normalized, { useSampleFallback: true });
    syncFieldsFromMarket(marketState);
    await loadNews(0, { useSampleFallback: true, asOf: normalized });
    await runAgent();
  } catch (error) {
    console.warn("Live hydration fallback to sample snapshot:", error);
  }
}

async function syncFromExternalDate(asOfDate) {
  const normalized = normalizeDateInput(asOfDate) || getDefaultTradingDate();
  pendingExternalDate = normalized;
  buildInitialSnapshot(normalized);
  void hydrateLiveData(normalized);
}

window.addEventListener("message", (event) => {
  const data = event.data;
  if (!data || typeof data !== "object") return;
  if (data.type !== "combined-date-sync" && data.type !== "commander-date-sync") return;
  void syncFromExternalDate(data.asOfDate);
});

async function boot() {
  sampleData = await loadSampleData();
  wireEvents();
  const initialDate = buildInitialSnapshot(pendingExternalDate || getDefaultTradingDate());
  void hydrateLiveData(initialDate);

  const beginnerQuestion = byId("beginner-question");
  if (sampleData.beginnerExamples.length && beginnerQuestion) {
    beginnerQuestion.value = sampleData.beginnerExamples[0];
    readQuestionAnswer();
  }
}
