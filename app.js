...
import { mountRunControl } from "./src/run-control.mjs";
...
// ---- Run-Control controller: LLM 오케스트레이터가 호출하는 "앱 동작 + 상태" 인터페이스 ----
// LLM은 숫자를 만들지 않는다. 아래 동작으로 규칙 기반 엔진을 실행하고 결과(getState)를 받아 인용만 한다.

const INPUT_FIELD_IDS = {
  oneDay: "input-oneDay",
  oneWeek: "input-oneWeek",
  oneMonth: "input-oneMonth",
  volatility20d: "input-volatility20d",
...
function getRunControlState() {
  ...
  return {
    ready: true,
    asOf: analysis.input?.asOf || marketState?.asOf || null,
    scenario: currentScenario,
    marketSource: marketState?.source || null,
    inputs: {
      returns: analysis.input?.returns,
      volatility20d: analysis.input?.volatility20d,
      trendStrength: analysis.input?.trendStrength,
      eventRisk: analysis.input?.eventRisk,
      newsSentiment: analysis.input?.newsSentiment,
      breadth: analysis.input?.breadth,
      slem: analysis.input?.slem,
      gelmanRubin: analysis.input?.gelmanRubin,
      tradabilityMaskValid: analysis.input?.tradabilityMaskValid
    },
    regime: analysis.regime?.label,
    marketInterpretation: analysis.marketInterpretation,
    regimeReason: analysis.regimeReason,
    confidence: analysis.confidence?.score,
    confidenceLabel: analysis.confidence?.label,
    topRecommended: (s.recommended || []).slice(0, 3).map((i) => ({
      name: i.name, strategyScore: i.strategyScore, riskLevel: i.riskLevel, expectedReturn: i.expectedReturn
    })),
    avoidNow: (s.avoidNow || []).slice(0, 3).map((i) => ({ name: i.name, rejectReason: i.rejectReason })),
    allocation: s.allocation || {},
    validation: analysis.validation ? {
      status: analysis.validation.status,
      statusLabel: analysis.validation.statusLabel,
      score: analysis.validation.score,
      strategyName: analysis.validation.strategyName,
      riskWarning: analysis.validation.riskWarning
    } : null,
    attribution: analysis.attribution ? {
      allocationEffect: analysis.attribution.allocationEffect,
      selectionEffect: analysis.attribution.selectionEffect,
      interactionEffect: analysis.attribution.interactionEffect,
    ...
    committee: analysis.committee ? {
      ...
      comment: analysis.committee.comment
    } : null,
    news: {
      sentiment: news.sentiment || null,
      keywords: (news.keywords || []).slice(0, 8),
      top: newsItems.slice(0, 3).map((i) => i.title)
    }
  };
}
...
const runControlController = {
  ...
  async setInputs(fields = {}) {
    for (const [key, id] of Object.entries(INPUT_FIELD_IDS)) {
      if (fields[key] === undefined || fields[key] === null) continue;
      const node = byId(id);
      if (node) node.value = fields[key];
    }
    if (fields.tradabilityMaskValid !== undefined) {
      const node = byId("input-tradabilityMaskValid");
      if (node) node.checked = Boolean(fields.tradabilityMaskValid);
    }
    currentScenario = "custom";
    setText("scenario-status", "Custom");
    renderScenarioButtons();
  ...
  answerQuestion(question) {
    const box = byId("beginner-question");
    if (box) box.value = String(question || "");
...
async function boot() {
  ...
  // LLM Run-Control 제어창 마운트 (boot 성공 후 1회)
  try {
    mountRunControl(runControlController);
  } catch (error) {
    console.warn("Run-Control mount skipped:", error);
  }
}
